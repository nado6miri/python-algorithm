...
class AndroidPhoneCtrl:
    ...
    * daemon started successfully
    ...
    LMV500N086051d6        unauthorized usb:34603008X transport_id:1
    R3CN30CWV2H            unauthorized usb:34603008X transport_id:3
    LMV500N086051d6        device usb:34603008X product:flashlmdd_lao_com model:LM_V500N device:flashlmdd transport_id:2
    R3CN30CWV2H            device usb:34603008X product:y2qksx model:SM_G986N device:y2q transport_id:4


    device_info = ['R3CN30CWV2H            ', ' usb:34603008X product:y2qksx model:SM_G986N ', ':y2q transport_id:4\n'], ids = R3CN30CWV2H, product = y2qksx, model = SM_G986N, manufacturer = samsung
    '''
    def get_phone_info(self):
        ...
        for resp_line in resp_lines:
            if "List of" not in resp_line and "device" in resp_line:
                ...
                print("device_id = {0}, product = {1}, model = {2}, manufacturer = {3}".format(ids, product, model, manufacturer))
...
class AndroidPhone:
    ...
    '''
    adb_shell_cmd = 'adb -s device_id shell '
    '''
    def setup(self):
        self.adb_shell_cmd = "adb -s {} shell ".format(self.device_id)

    '''
    adb -s device_id reboot 을 실행한다.
    '''
    def reboot(self):
    ...
    '''
    화면이 잠금상태일때 암호를 풀어 사용모드로 들어가는 함수로 보면 됨
    power key + 3 sec delay + Wake Up event + 3 sec delay + touch slide 
    '''
    def wakeup_and_screen_on(self):
    ...
    '''
    현재 화면을 screen shot을 해서 특정 위치에 저장하는 함수
    화면저장 : adb -s device_id shell screencap /sdcard/filename
    파일가져오기 : adb -s device_id pull /sdcard/filename
    '''
    ...
    '''
    input box에 text를 입력함.
    adb -s device_id shell input text "description"
    '''
    def input_text(self, desc):
    ...
    '''
    x, y 좌표에 화면을 터치함.
    adb -s device_id shell input tab x y
    '''
    def touch_screen(self, x, y):
...
1. Android phone class 를 상속받아 samsung / LG class를 왜 만들었는지? vendor별로 차이점이 있어서 만들었는지...
